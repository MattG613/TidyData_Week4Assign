...
into two different sets, training and test data.

This function reads in the data, and outputs a tidy data summary of the mean values
for all collected data involving mean or standard deviation.
...
(y_*.txt), then the subject information (subject_*).

These tables are all combined into one data set using cbind and rbind.

Then, the actual names for each column in the data set (the real data) are read
...
These variable names, along with Subject and Activity are used to name all of the 
columns in the subsetted tidy.


Even though it is subsetted the data is not yet "tidy" as requested, since we have multiple entries for activities for each subject.
